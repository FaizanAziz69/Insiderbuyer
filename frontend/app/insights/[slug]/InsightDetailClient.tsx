...
/**
 * The article view. Rendered on the server by `page.tsx`, which fetches the
 * post and the rail list from the backend and passes them in as SWR fallback
 * data — so the headline, standfirst and full body are in the HTML a crawler
 * receives (George, 2026-09-09: "everything is being rendered client side …
 * Google will not recognize our content"). SWR then owns the data on the
 * client exactly as before: same keys, same revalidation, so nothing about the
 * interactive behaviour changed. When the server fetch fails, the props are
 * null and this falls back to the old client-only load.
 */
export default function InsightDetailClient({
  ...
  initialLatest?: BlogListResponse | null;
}) {
  ...
  const { data: post, isLoading, error } = useSWR<BlogPost>(
    ...
    fetcher,
    {
      ...
      // The server already fetched it moments ago; don't refetch on mount just
      // to get the same bytes back. Navigating between articles still fetches
      // (the key changes).
  ...
  const { data: latest } = useSWR<BlogListResponse>(
    ...
    fetcher,
    {
      ...
      fallbackData: initialLatest ?? undefined,
    },
  );
  const railItems =
